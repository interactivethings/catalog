...
class Menu extends React.Component {
  ...
  setFilterString(e) {
    ...
    this.props.runSearch(filterString)
      .then(matches => {
        this.setState({
  ...
  render() {
    ...
    const pages = filterString
      ? filteredPages.map(page => <ListItem key={page.id} page={page} theme={theme} />)
    ...
    return (
      <div style={currentStyle.bar} >
        <div style={{flexGrow: 1}}>
          ...
          <input style={currentStyle.search} placeholder='Filter pages' type='search' value={filterString} onChange={e => this.setFilterString(e)}/>
          <ul style={currentStyle.list} onClick={() => this.resetFilterString()}>
            {pages}
          </ul>
        </div>
